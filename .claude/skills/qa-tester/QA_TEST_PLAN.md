...
| `Baseline` | 标准数据：default 集合(simple.pdf + with_images.pdf)、test_col 集合(complex_technical_doc.pdf)、有 Trace | `python {{SKILL_ROOT}}/scripts/qa_bootstrap.py` |
...
| K-09 | DeepSeek 回退 Azure 验证 | DeepSeek | 1. 执行 `python {{SKILL_ROOT}}/scripts/qa_config.py restore`<br>2. 执行 `python scripts/query.py --query "Sample Document PDF loader" --verbose` | 功能恢复正常，Verbose 输出显示使用 Azure LLM，查询结果包含 simple.pdf 相关内容，验证切换回来无副作用 |
