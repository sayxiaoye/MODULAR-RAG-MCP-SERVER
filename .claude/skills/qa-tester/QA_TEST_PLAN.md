...
- [QA 专项测试计划 — Modular RAG MCP Server](#qa-专项测试计划--modular-rag-mcp-server)
  - [目录](#目录)
  - [系统状态定义](#系统状态定义)
  ...
  - [附录：测试环境准备清单](#附录测试环境准备清单)
    - [配置文件备份](#配置文件备份)
    - [测试文档](#测试文档)
    - [API Key 准备](#api-key-准备)
      - [外部 API Key 配置步骤](#外部-api-key-配置步骤)
      - [配置切换方式](#配置切换方式)
...
| `Baseline` | 标准数据：default 集合(sample.pdf + with_images.pdf)、test_col 集合(complex_technical_doc.pdf)、有 Trace | `python {{SKILL_ROOT}}/scripts/qa_bootstrap.py` |
...
| B-03 | 选择集合后文档列表展示 | Baseline | 1. 下拉框选择 "default" | 页面展示 2 个文档条目（sample.pdf 和 with_images.pdf），每个显示文件名、chunk 数、图片数。sample.pdf 图片数=0，with_images.pdf 图片数≥1 |
| B-04 | 展开文档查看 Chunk 详情 | Baseline | 1. 点击 sample.pdf 文档的展开箭头 | 展示 sample.pdf 的所有 Chunk（数量 ≥ 1），每个 Chunk 显示文本内容（只读文本框，应含 "Sample Document" 相关内容）和 Metadata 展开按钮 |
| B-05 | 查看 Chunk Metadata | Baseline | 1. 展开 sample.pdf 的第一个 Chunk<br>2. 点击"📋 Metadata"展开 | 显示 JSON 格式的 metadata，包含 source 字段（值含 "sample.pdf"）、doc_hash（SHA256 哈希）、title、tags 等字段 |
| B-06 | 查看关联图片预览 | Baseline | 1. 展开含图片的文档(with_images.pdf)<br>2. 查看图片预览区域 | 页面显示图片缩略图（最多 4 列网格排列） |
| B-07 | 切换集合后文档列表刷新 | Baseline | 1. 下拉框从 default 切换到 test_col | 文档列表刷新，显示 test_col 集合的文档（包含 complex_technical_doc.pdf），不再显示 default 的 sample.pdf/with_images.pdf |
...
| C-02 | 上传 PDF 文件并摄取 | Baseline | 1. 点击文件上传区域，选择 `tests/fixtures/sample_documents/sample.pdf`<br>2. 集合名保持 "default"<br>3. 点击"🚀 Start Ingestion" | 进度条从 0% 推进，依次显示 integrity→load→split→transform→embed→upsert 各阶段，最终显示成功提示 |
| C-03 | 摄取完成后文档出现在列表 | Baseline | 1. 查看下方文档列表 | 列表中出现 sample.pdf 条目，显示 chunk 数量 > 0 |
...
| C-06 | 重复摄取同一文件（幂等性） | Baseline | 1. 再次上传 sample.pdf 到 default 集合<br>2. 点击"🚀 Start Ingestion" | 系统检测到文件已处理（SHA256 匹配），跳过处理或快速完成，不产生重复 Chunk |
| C-07 | 强制重新摄取 | Baseline | 1. 在文档列表中删除 sample.pdf<br>2. 再次上传 sample.pdf 并摄取 | 重新处理全流程，Chunk 重新生成 |
| C-08 | 删除单个文档 | Baseline | 1. 在 default 集合中找到 sample.pdf 条目<br>2. 点击 sample.pdf 旁的"🗑️ Delete"按钮 | sample.pdf 从列表消失，显示成功提示。跨 4 个存储（Chroma、BM25、Images、FileIntegrity）均已清理 |
| C-09 | 删除文档后查询验证 | Baseline | 1. 承接 C-08 删除 sample.pdf 后<br>2. 执行 `python scripts/query.py --query "Sample Document PDF loader" --verbose` | 查询结果不再包含来源为 sample.pdf 的 Chunk，source_file 字段中无 sample.pdf |
...
| D-05 | Load 阶段 Tab 详情 | Baseline | 1. 展开 sample.pdf 的 Trace<br>2. 点击"📄 Load"Tab | 显示 Doc ID、Text Length（> 0）、Images 数量（sample.pdf 为 0）指标，以及 Raw Text 预览（应含 "Sample Document" 文本） |
...
| ID | 测试标题 | 状态 | 操作步骤 | 预期现象 |
|----|---------|------|---------|--------- |
| G-01 | 摄取单个 PDF 文件 | Baseline | 1. 执行 `python scripts/ingest.py --path tests/fixtures/sample_documents/sample.pdf` | 控制台输出各阶段处理信息，最终显示摄取成功，exit code=0 |
...
| G-03 | 指定集合名摄取 | Baseline | 1. 执行 `python scripts/ingest.py --path sample.pdf --collection test_col` | 文件摄取到 test_col 集合，可在 Dashboard Data Browser 中看到 |
...
| G-05 | --force 强制重新处理 | Baseline | 1. 执行 `python scripts/ingest.py --path sample.pdf --force` | 跳过 SHA256 检查，强制重新处理全流程 |
| G-06 | 重复摄取（无 --force） | Baseline | 1. 执行 `python scripts/ingest.py --path sample.pdf`（不加 --force） | 控制台提示文件已处理/跳过，不产生重复 Chunk |
| G-07 | --verbose 详细输出 | Baseline | 1. 执行 `python scripts/ingest.py --path sample.pdf --verbose` | 输出 DEBUG 级别日志，包含各阶段详细信息 |
| G-08 | 指定配置文件 | Baseline | 1. 执行 `python scripts/ingest.py --path sample.pdf --config config/settings_test.yaml` | 使用指定配置文件的设置进行摄取 |
...
| H-03 | 指定集合查询 | Baseline | 1. 执行 `python scripts/query.py --query "Retrieval-Augmented Generation" --collection test_col` | 仅从 test_col 中检索，结果全部来自 complex_technical_doc.pdf，不混入 default 集合的 sample.pdf/with_images.pdf |
...
| J-05 | get_document_summary 功能 | Baseline | 1. 触发 get_document_summary 工具调用，传入 doc_id 为 sample.pdf 的文档 ID（可先通过 list_collections 获取） | 返回 sample.pdf 的 title（应含 "Sample Document"）、summary、tags 元信息，内容与文档实际内容匹配 |
...
| J-10 | 引用透明性检查 | Baseline | 1. 调用 query_knowledge_hub，query="Sample Document PDF loader"，collection="default"<br>2. 检查返回结果的引用信息 | 每个检索片段包含 source_file（值为 "sample.pdf"）、page（值为 1）、chunk_id（非空字符串）、score（分数在 0-1 之间），支持溯源 |
...
| ID | 测试标题 | 状态 | 操作步骤 | 预期现象 |
|----|---------|------|---------|--------- |
| K-01 | settings.yaml 切换 LLM 到 DeepSeek | DeepSeek | 1. 执行 `python {{SKILL_ROOT}}/scripts/qa_config.py apply deepseek`<br>2. 检查输出确认切换成功 | 输出显示 "LLM -> deepseek / deepseek-chat"，settings.yaml 已更新 |
...
| K-03 | DeepSeek LLM — 摄取（Chunk Refiner） | DeepSeek | 1. 执行 `python scripts/ingest.py --path tests/fixtures/sample_documents/sample.pdf --force --verbose` | Transform 阶段使用 DeepSeek 进行 Chunk 重写，日志可见。重写后的 Chunk 内容合理、中文通顺 |
...
| K-09 | DeepSeek 回退 Azure 验证 | DeepSeek | 1. 执行 `python {{SKILL_ROOT}}/scripts/qa_config.py restore`<br>2. 执行 `python scripts/query.py --query "Sample Document PDF loader" --verbose` | 功能恢复正常，Verbose 输出显示使用 Azure LLM，查询结果包含 sample.pdf 相关内容，验证切换回来无副作用 |
...
| M-02 | Azure Embedding API key 错误 | InvalidEmbedKey | 1. 将 `embedding.api_key` 改为无效值<br>2. 执行 `python scripts/ingest.py --path tests/fixtures/sample_documents/sample.pdf --force` | 在 Embed 阶段报错，错误信息明确指向 Embedding API 问题（如 "401 Unauthorized" 或 "Invalid API key"） |
...
| M-10 | Chunk Size 参数调整 | Baseline | 1. 将 `ingestion.chunk_size` 从 1000 改为 500<br>2. 执行 `python scripts/ingest.py --path tests/fixtures/sample_documents/sample.pdf --force`<br>3. 在 Dashboard Data Browser 查看 sample.pdf 的 Chunk 数量 | 生成更多更短的 Chunk（数量应比 chunk_size=1000 时多），每个 Chunk 文本长度 ≤ 500 字符（约） |
| M-11 | Chunk Overlap 参数调整 | Baseline | 1. 将 `ingestion.chunk_overlap` 从 200 改为 0<br>2. 执行 `python scripts/ingest.py --path tests/fixtures/sample_documents/sample.pdf --force`<br>3. 在 Dashboard Data Browser 展开 sample.pdf 的相邻 Chunk | Chunk 之间无重叠文本（相邻 Chunk 的开头不应与前一个 Chunk 的结尾重复） |
| M-12 | 关闭 LLM Chunk Refiner | Baseline | 1. 将 `ingestion.chunk_refiner.provider` 改为非 LLM 模式（如 rule-based）<br>2. 执行 `python scripts/ingest.py --path tests/fixtures/sample_documents/sample.pdf --force --verbose`<br>3. 在 Dashboard Data Browser 查看 Chunk 内容 | Transform 使用规则方式（非 LLM）精简 Chunk，摄取速度更快（日志无 LLM 调用记录），Chunk 文本与原始 Split 结果更接近 |
| M-13 | 关闭 LLM Metadata Enricher | Baseline | 1. 将 `ingestion.metadata_enricher.provider` 改为非 LLM 模式（如 rule-based）<br>2. 执行 `python scripts/ingest.py --path tests/fixtures/sample_documents/sample.pdf --force`<br>3. 在 Dashboard Data Browser 查看 sample.pdf 的 Chunk Metadata | Metadata 中 title/summary/tags 由规则方式生成（可能为空或简略），无 LLM 增强内容，summary 不会是 LLM 编写的自然语言摘要 |
...
| ID | 测试标题 | 状态 | 操作步骤 | 预期现象 |
|----|---------|------|---------|--------- |
| N-01 | 完整闭环: 摄取→查询→删除→查询 | Empty | 1. 执行 `python scripts/ingest.py --path tests/fixtures/sample_documents/sample.pdf`<br>2. 执行 `python scripts/query.py --query "Sample Document PDF loader"` → 确认命中<br>3. 在 Dashboard Data Browser 删除 default 集合中的 sample.pdf<br>4. 再次执行 `python scripts/query.py --query "Sample Document PDF loader"` | 步骤 2 返回结果，source_file 含 sample.pdf；步骤 4 不再返回 sample.pdf 相关结果（结果为空或仅含其他文档） |
| N-02 | 删除后重新摄取 | Baseline | 1. 在 Dashboard 删除 default 集合中的 sample.pdf<br>2. 执行 `python scripts/ingest.py --path tests/fixtures/sample_documents/sample.pdf`<br>3. 执行 `python scripts/query.py --query "Sample Document PDF loader"` | 摄取成功（FileIntegrity 记录已清理，不会被跳过），查询重新命中 sample.pdf 的 Chunk |
| N-03 | 多集合隔离验证 | Baseline | 1. 执行 `python scripts/ingest.py --path tests/fixtures/sample_documents/sample.pdf --collection isolate_a`<br>2. 执行 `python scripts/ingest.py --path tests/fixtures/sample_documents/complex_technical_doc.pdf --collection isolate_b`<br>3. 执行 `python scripts/query.py --query "Sample Document PDF loader" --collection isolate_a`<br>4. 执行 `python scripts/query.py --query "Retrieval-Augmented Generation" --collection isolate_b` | 集合 isolate_a 查询仅返回 sample.pdf 内容（source_file=sample.pdf）；集合 isolate_b 查询仅返回 complex_technical_doc.pdf 内容，互不干扰 |
| N-04 | Clear All Data 后全功能验证 | Baseline | 1. Dashboard Clear All Data<br>2. 执行 `python scripts/query.py --query "Sample Document"` → 无结果<br>3. 执行 `python scripts/ingest.py --path tests/fixtures/sample_documents/sample.pdf`<br>4. 执行 `python scripts/query.py --query "Sample Document PDF loader"` → 命中 | 清空后查询返回空；重新摄取 sample.pdf 后查询命中，系统完全恢复正常 |
| N-05 | 同一文件摄取到多个集合 | Baseline | 1. 摄取 sample.pdf 到 collection_1<br>2. 再次摄取 sample.pdf 到 collection_2 | 两个集合各自独立拥有该文档的 Chunk，互不影响 |
| N-06 | 删除集合 A 中的文档不影响集合 B | Baseline | 1. 在 Dashboard 删除 collection_1 中的 sample.pdf<br>2. 查询 `--collection collection_2` | collection_2 中的数据不受影响，仍可查询命中 |
...
# 步骤 3: 验证配置
python {{SKILL_ROOT}}/scripts/qa_config.py check
```
...
```powershell
# 查看可用 Profile
python {{SKILL_ROOT}}/scripts/qa_config.py show

# 切换到 DeepSeek（自动备份 settings.yaml，注入 API Key）
python {{SKILL_ROOT}}/scripts/qa_config.py apply deepseek

# 执行测试...

# 测试完成后恢复原始配置
python {{SKILL_ROOT}}/scripts/qa_config.py restore
```
